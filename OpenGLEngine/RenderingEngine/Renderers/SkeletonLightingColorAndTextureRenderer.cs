...
{
    public class SkeletonLightingColorAndTextureRenderer : SkeletonRenderer
    {
        int shapeData;
        int indiceData;
        int triangleCount;
        int textureID;
        SkeletonLightingColorAndTextureProgram program;
        Camera camera;
        Engine engine;

        public SkeletonLightingColorAndTextureRenderer(int shapeData, int indiceData, int textureID, int triangleCount, Engine engine)
        {
            this.shapeData = shapeData; this.indiceData = indiceData; this.triangleCount = triangleCount;
            this.camera = engine.camera; program = engine.programList.SkeletonLightingColorAndTextureProgram;
        ...
        public void Render(Matrix4 modelMatrix, Matrix4[] skeleton)
        {
            ...
            GL.VertexAttribPointer(program.positionHandle, 3, VertexAttribPointerType.Float, false, 64, 0);

            GL.EnableVertexAttribArray(program.normalHandle);
            GL.VertexAttribPointer(program.normalHandle, 3, VertexAttribPointerType.Float, false, 64, 12);

            GL.EnableVertexAttribArray(program.textureHandle);
            GL.VertexAttribPointer(program.textureHandle, 2, VertexAttribPointerType.Float, false, 64, 24);

            GL.EnableVertexAttribArray(program.colorHandle);
            GL.VertexAttribPointer(program.colorHandle, 4, VertexAttribPointerType.Float, false, 64, 32);
            
            GL.EnableVertexAttribArray(program.boneIndexHandle);
            GL.VertexAttribPointer(program.boneIndexHandle, 2, VertexAttribPointerType.Float, false, 64, 48);
            
            GL.EnableVertexAttribArray(program.boneWeightHandle);
            GL.VertexAttribPointer(program.boneWeightHandle, 2, VertexAttribPointerType.Float, false, 64, 56);
